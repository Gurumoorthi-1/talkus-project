...
const Sidebar = () => {
  ...
  useEffect(() => {
    // Detect if device is mobile on load
  ...
  const filteredUsers = input
    ? users.filter((user) =>
        `${user.fullName || user.username || user.name || "Unknown"}`
          .toLowerCase()
      ...
      <div className="pb-5 flex justify-between items-center">
        ...
        <div
          className={`relative py-2 ${
            !isMobile ? "group" : "" // enable hover only for desktop
          }`}
        >
          <img
            ...
            onClick={() => isMobile && setMenuOpen(!menuOpen)} // mobile click only
          />
          ...
          <div
            className={`absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] 
              ...
              ${isMobile ? (menuOpen ? "block" : "hidden") : "hidden group-hover:block"}`}
          >
            <p
              onClick={() => {
                setMenuOpen(false);
